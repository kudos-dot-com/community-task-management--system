...
function AddCampusAmbassador({data}) {
    ...
    const [tableitem,settableitem]=useState(["number","email","city","state","country","residence"].sort());
    ...
    useEffect(()=>{
        ...
        fetch(`/api/users/org-get-campus-user`,{    
        headers:{
        ...
        .then(data =>{
            console.log(data);
            if(data.err==='you must be logged in')
            {
             history.push('/')
            }
